...
let render_todo toggle_path delete_path (id, text, done_) =
  Html.el "div"
    ~attrs:[ ("class", if done_ then "todo done" else "todo") ]
    [
      Html.el "span" [ Html.text text ];
      Html.el "button"
        ~attrs:
          [
            ("data-on:click", Printf.sprintf "@post('%s?id=%s')" toggle_path id);
          ]
        [ Html.text (if done_ then "Undo" else "Done") ];
      Html.el "button"
        ~attrs:
          [
            ("data-on:click", Printf.sprintf "@post('%s?id=%s')" delete_path id);
          ]
        [ Html.text "Delete" ];
    ]
...
let () =
  Lwt_main.run
    (let open Lwt.Syntax in
     ...
     let add_path =
       Action.define hub "add_todo" (fun request ->
           ...
           if String.length text > 0 then (
             ...
             incr next_id;
             let value =
               `Assoc [ ("text", `String text); ("done", `Bool false) ]
             in
             let* () =
               Store.set store [ "todos"; id ] (Yojson.Safe.to_string value)
             in
             Lwt.return
               (Action.patch_signals (`Assoc [ ("todo_input", `String "") ])))
           else Lwt.return Action.no_content)
     in

     let toggle_path =
       Action.define hub "toggle_todo" (fun request ->
           ...
           | Some id -> (
               ...
               | Some json_str ->
                   ...
                   in
                   let* () =
                     Store.set store [ "todos"; id ]
                       (Yojson.Safe.to_string toggled)
                   in
                   Lwt.return Action.no_content))
     in

     let delete_path =
       Action.define hub "delete_todo" (fun request ->
           match get_query_param request.Router.query "id" with
           | None -> Lwt.return Action.no_content
           | Some id ->
     ...
     let _view_path =
       View.define
         ...
         ~datastar_js hub "/" (fun _request ->
           ...
           in
           let todo_html =
             Html.concat (List.map (render_todo toggle_path delete_path) todos)
           in
           let body =
